...
enum RequestCommand {
    SetActivity(SetActivityRequest),
    Subscribe(SubscribeRequest),
    Unsubscribe(UnsubscribeRequest),
}

fn main() {
    let command = RequestCommand::Unsubscribe(UnsubscribeRequest {
        nonce: "f48f6176-4afb-4c03-b1b8-d960861f5216".to_string(),
        args: UnsubscribeArgs { guild_id: "123".to_string() },
    });
    let bytes = serde_json::to_string(&command).unwrap();
    println!("{}", bytes);
    let command = serde_json::from_str::<RequestCommand>(&bytes).unwrap();
    assert!(matches!(command, RequestCommand::Unsubscribe(_)));
    println!("{:?}", command);
    ...
    "#;
    let command = serde_json::from_str::<RequestCommand>(payload).unwrap();
    println!("{:?}", command);
    // let payload = Payload {
    //     nonce: "f48f6176-4afb-4c03-b1b8-d960861f5216".to_string(),
    //     args: PayloadArgs {
    ...
    // let payload = Payload { v: 1, client_id:
    // "1276759902551015485".to_string() }; let payload_disconnect_activity
    // = serde_json::to_vec(&PayloadDisconnectActivity {
    //     cmd: "SET_ACTIVITY".to_string(),
    //     args: EmptyDisconnectArgs { pid: std::process::id() },
